...
function checkCommunityAdmin(textData:string,comparisonString:string|undefined):[string,string,string]{
  let feedback;
  let validity;
  let buttonsStatus;
...
function ManageCommunity({ searchValue }:{searchValue:string}) {
  ...
  } = NamesRegistryReadHook({functionName:'getCommunityMembershipsCount',functionArgs:[searchValue]});
  ...
  } = NamesRegistryReadHook({functionName:'isSuspendedByAdmin',functionArgs:[communityHash]});
  ...
  useEffect(() => {
    if (dataMembershipsCount !== undefined && dataMembershipsCount.toString() !== membershipsCount) {
      setMembershipsCount(dataMembershipsCount.toString());
    }else if(isErrorMembershipsCount){
      refetchMembershipsCount();
    }
  }, [dataMembershipsCount,isErrorMembershipsCount]);

  useEffect(() => {
    if (dataIsSuspendedByAdmin !== undefined && dataIsSuspendedByAdmin.toString() !== isSuspendedByAdmin.toString()) {
  ...
  useEffect(() => {
    if (dataIsCommunityVerified !== undefined && dataIsCommunityVerified.toString() !== isCommunityVerified) {
  ...
  useEffect(() => {
    if (dataCommunitySearch !== undefined) {
      let [status,validity,buttonsStatus] = checkCommunityAdmin(dataCommunitySearch?.toString(),address?.toString());
      ...
      <>
        {[ 'light',
          ...
          <Alert key={variant} variant={variant}>
            <div>
              <p> Membership cards issued: <b> {isLoadingMembershipsCount || isErrorMembershipsCount ? "loading..." : membershipsCount}</b> </p>
              <p> Is community suspended by Admin: <b>{isLoadingIsSuspendedByAdmin || isErrorIsSuspendedByAdmin ? "loading..." : isSuspendedByAdmin}</b> </p>
              <p> Is community verified: <b>{isLoadingIsCommunityVerified || isErrorIsCommunityVerified ? 'loading...' : isCommunityVerified}</b> </p>
            </div>
          </Alert>
        ))}
      </>
      <ModifyCommunityAdmin currentAdminAddress = {dataCommunitySearch?.toString()} buttonsStatus={buttonsStatus} searchValue={searchValue} />
      <SuspendUnsuspendCommunity searchValue = {searchValue} buttonsStatus = {buttonsStatus} isSuspendedByAdmin = {isSuspendedByAdmin} />
      </Form>
    </div>
  );
}
